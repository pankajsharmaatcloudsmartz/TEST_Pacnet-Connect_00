...
<style>
.aui-label-required{display:none;}
</style>
<%
...
<div class="title" style="max-width:527px; margin:0 auto;"><h5 class="signin-txt">Forgot Password </h5></div>
	...
	<aui:fieldset>
		<c:choose>
			<c:when test="<%= user2 == null %>">
				...
				<input type="submit" value='<%= PropsValues.USERS_REMINDER_QUERIES_ENABLED ? "Next" : "send-new-password" %>' class="btn btn-primary"/>
				<input type="button" value='cancel' onclick="location.href='/'" class="btn btn-primary" />
			</c:when>
			<c:when test="<%= (user2 != null) && Validator.isNotNull(user2.getEmailAddress()) %>">
				...
				<c:choose>
					...
					<c:otherwise>
						...
						<input type="submit" value='<%= company.isSendPasswordResetLink() ? "Send-password-reset-link" : "send-new-password" %>' class="btn btn-primary"/>
						<input type="button" value='cancel' onclick="location.href='/'" class="btn btn-primary" />
					</c:otherwise>
				</c:choose>
			</c:when>
